...
function $mktmp(type_name: type_name): string {
  ...
  while (
    key in variable_table.table &&
    to_wasm_type(variable_table.table[key]!) !== to_wasm_type(type_name)
  ) {
    tmpidx.increment();
...
function translate_literal(literal_node: ASTNode): Array<string> {
  ...
  const output = code.o;
  if (val_type === "int") {
    return [`i32.const ${code.lhs}`, `local.set $${output}`];
  } else if (val_type === "float") {
    return [`f32.const ${code.lhs}`, `local.set $${output}`];
  } else if (val_type === "char") {
    return [`i32.const ${code.lhs.charCodeAt(0)}`, `local.set $${output}`];
  } else if (val_type === "bool") {
    return [`i32.const ${code.lhs ? 1 : 0}`, `local.set $${output}`];
  } else if (val_type === "void") {
    return [`i32.const 0`, `local.set $${output}`];
  } else {
...
function translate_expr(expr_node: ASTNode): Array<string> {
  ...
  if (op === "get") {
    ...
    const target = code.o;
    return [`local.get $${source}`, `local.set $${target}`];
    ...
    if (call_op.args.length === 0) {
      return [`call $${call_op.name}`, `local.set $${call_op.o}`];
    }
    ...
    result.push(...[`call $${call_op.name}`, `local.set $${call_op.o}`]);
    return result;
  }
...
let cur_block_label: BlockLabel = {
  value: "",
  parent: null,
};
...
function translate_if(if_node: ASTNode): Array<string> {
  ...
  const translate_false_branch = () => {
    if (f_type === "nest") {
      return [...translate_if(branch_node.children[code_f]!.children[0]!)];
    } else if (f_type === "else") {
      return [...translate_block(branch_node.children[code_f]!.children[0]!)];
    } else if (f_type === "no") {
      return [];
    } else {
  ...
  const combine_branch = (
    true_branch: Array<string>,
    false_branch: Array<string>,
  ) => {
    if (f_type == "no") {
      return [`(then`, ...true_branch, `)`];
    } else if (f_type == "else" || f_type == "nest") {
      return [`(then`, ...true_branch, `)`, `(else`, ...false_branch, `)`];
    } else {
      throw new Error(`unknown f_type: ${f_type}`);
    }
  };
...
function translate_stmt(stmt_node: ASTNode): Array<string> {
  ...
  if (code_prop.op === "pass") {
    ...
    return [`br $${cur_block_label.value}_br_loop`];
  } else if (code_prop.op === "continue") {
    return [`br $${cur_block_label.value}`];
    ...
    return [...translated_expr, `local.get $${ptr_value}`, `call $rt_free`];
...
function translate_func(func_node: ASTNode): string {
  ...
  const param = func.param.map(
    (param) => `(param $${param.name} ${to_wasm_type(param.type_name)})`,
  );
  ...
  for (const decl_info of var_decl_info) {
    ...
    decl_code.push(
      `(local $${decl_info.name} ${to_wasm_type(decl_info.type_name)})`,
    );
  }
  const code = Array<string>();

  code.push(...translate_block(block_node));
...
function translate(root: ASTNode): string {
  ...
  const import_func = ["i32", "i64", "f32", "f64"].map(
    (e) => `(import "runtime" "echo" (func $rt_echo_${e} (param ${e})))`,
  );
