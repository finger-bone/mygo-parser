...
void SLR1Parser::print_parse_table() const {
  ...
  for (const auto &prod : productions) {
    ...
    if (std::find(non_terminals.begin(), non_terminals.end(), nt) ==
        non_terminals.end()) {
    ...
    for (const auto &symbol : prod.right) {
      if (is_terminal(symbol.type) &&
          std::find(terminals.begin(), terminals.end(), symbol) ==
              terminals.end()) {
  ...
  // 打印分割线
  std::cout << std::string(
                   5 + terminals.size() * 10 + non_terminals.size() * 10, '-')
            << std::endl;
    ...
    for (const auto &terminal : terminals) {
      if (action_table.find(i) != action_table.end() &&
        ...
        std::cout << std::setw(10)
                  << action_table.at(i).at(terminal).to_string();
      } else {
        std::cout << std::setw(10) << "";
      }
    }
