...
namespace slr {
    ...
    std::string LR0Item::to_string() const {
        ...
        for (size_t i = 0; i < production.size(); ++i) {
            if (i == dot_position) ss << ".";
            ss << production[i].to_string() << " ";
        }
        if (dot_position == production.size()) ss << ".";
        return ss.str();
    }
    ...
    bool SLR1Parser::parse(const std::vector<SLRSymbol>& input, ParserTreeNode& root) {
        ...
        while (true) {
            ...
            if (action_table[current_state].find(current_symbol) == action_table[current_state].end()) {
                std::cerr << "语法错误：状态" << current_state << "，符号" << current_symbol.to_string() << "，token编号" << input_pos << std::endl;
                return false;
            }
            ...
            switch (action.type) {
                case ActionType::SHIFT: {
                    ...
                    symbol_stack.push(ParserTreeNode(current_symbol));
                    input_pos++;
                    break;
                }
                case ActionType::REDUCE: {
                    // 规约
                    int prod_index = action.value;
