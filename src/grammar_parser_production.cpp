...
// 只识别被单引号、双引号或尖括号包裹的
// token，忽略其它空白（除换行外）以及无效字符
std::optional<ProductionList> ProductionList::parse(const std::string &str) {
  if (str.empty()) {
    std::cerr << "Error: Production list cannot be an empty string"
              << std::endl;
    return std::nullopt;
  }
  ...
  while (i < str.size()) {
    ...
    // 单引号：终结符
      ...
      if (i >= str.size()) {
        std::cerr << "Error: Terminal inside single quotes must be closed with "
                     "\'\'\'. Given: "
                  << str.substr(start) << std::endl;
        return std::nullopt;
      }
    ...
    // 双引号：变量（非终结符）
      ...
      if (i >= str.size()) {
        std::cerr << "Error: NonTerminal inside double quotes must be closed "
                     "with '\"'. Given: "
                  << str.substr(start) << std::endl;
        return std::nullopt;
      }
    ...
    // 尖括号：特殊终结符
      ...
      if (i >= str.size()) {
        std::cerr << "Error: Special terminal inside angle brackets must be "
                     "closed with '>'. Given: "
                  << str.substr(start) << std::endl;
        return std::nullopt;
      }
