...
// 执行规约操作
bool SLR1Parser::perform_reduce(int prod_index, std::stack<int>& state_stack, std::stack<CSTNode>& symbol_stack, size_t _) {
    ...
    // 创建新的CST节点
    CSTNode new_node(SLRSymbol(production.left, SLRSymbolType::NON_TERMINAL), children, production);
    ...
    if (goto_table.find(current_state) == goto_table.end() ||
        goto_table.at(current_state).find(non_terminal) == goto_table.at(current_state).end()) {
        return false;
    }
...
// 执行接受操作
bool SLR1Parser::perform_accept(std::stack<CSTNode>& symbol_stack, CSTNode& root, size_t _) {
...
bool SLR1Parser::handle_error(int state, const SLRSymbol& symbol, size_t input_pos) {
    std::cerr << "Syntax error at position " << input_pos << ": unexpected symbol " << symbol.to_string() << " in state " << state << std::endl;
    return false;
}
